...
async function run() {
  ...
  var points_waypoints = randomPointsOnPolygon(numberOfPoints+1, poly.features[0]);
  var destArray = [];
  var originArray = [];
  var wayPointArray = [];
  var steps = [];
  ...
  points_waypoints.forEach(function(geo) {
    wayPointArray.push(geo.geometry.coordinates[1].toFixed(precision) + ',' + geo.geometry.coordinates[0].toFixed(precision));
  })

  let departureTime = Math.round(new Date().getTime() / 1000);
  await axios.get(`${apiHost}/directions/json?key=${apiKey}&annotations=true&steps=true&alternatives=false&origin=${originArray[0]}&waypoints=${wayPointArray[0]}|${wayPointArray[0]}&destination=${destArray[0]}&mode=4w&departure_time=${departureTime}`)
    .then((res) => {
      res.data.routes.forEach(function(route) {
        const route_points = polyline.decode(route.geometry);
        console.log(`Origin = ${originArray[0]}`);
        console.log(`Waypoint 1 = ${wayPointArray[0]}`);
        console.log(`Waypoint 2 = ${wayPointArray[1]}`);
